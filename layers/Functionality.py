...
class MLPLayer(nn.Module):
    ...
    def forward(self, x):  # x(32, 64, 512)
        y = x = self.norm1(x)  # 层归一化
        y = self.dropout(self.activation(self.conv1(y.transpose(-1, 1))))  # (32,64,512)=>(32,512,64)=>(32,2048,64)
        if self.kernel_size != 1:
            y = y[..., 0:-(self.kernel_size - 1)]
        y = self.dropout(self.conv2(y).transpose(-1, 1))  # (32,64,512)
        if self.kernel_size != 1:
            y = y[:, 0:-(self.kernel_size - 1), :]
        if self.res_con:  # 使用残差连接
